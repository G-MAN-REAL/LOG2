...
class Paths
{
	...
	public static function shrunkImage(Source:String, scaleFactor:Float = 1):FlxGraphic
		{
			var oldmap = FlxG.bitmap.add(Source);
			var newmap = new BitmapData(Std.int(oldmap.width * scaleFactor), Std.int(oldmap.height * scaleFactor), true, 0x00000000);
			newmap.draw(oldmap.bitmap, new openfl.geom.Matrix(scaleFactor, 0, 0, scaleFactor, 0, 0));
			FlxG.bitmap.remove(oldmap);
			var graphic = FlxGraphic.fromBitmapData(newmap, false, 'shrunk/$Source');
			return graphic;
		}
		inline static public function shrunkSparrow(path:String, scaleFactor:Float = 1, ?Key:String)
		{
			var Description:String = getPath('images/$path.xml');
	
			var graphic:FlxGraphic = shrunkImage(getPath('images/$path.png', IMAGE), scaleFactor);
			if (graphic == null)
				return null;
			...
			for (texture in data.nodes.SubTexture)
			{
				...
				var rect = FlxRect.get(Std.parseFloat(texture.att.x) * scaleFactor, Std.parseFloat(texture.att.y) * scaleFactor,
					Std.parseFloat(texture.att.width) * scaleFactor, Std.parseFloat(texture.att.height) * scaleFactor);
	
				var size = if (trimmed)
				{
					new Rectangle(Std.parseInt(texture.att.frameX) * scaleFactor, Std.parseInt(texture.att.frameY) * scaleFactor,
						Std.parseInt(texture.att.frameWidth) * scaleFactor, Std.parseInt(texture.att.frameHeight) * scaleFactor);
				}
				else
				{
