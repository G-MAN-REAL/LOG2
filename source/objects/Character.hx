...
typedef CharacterFile = {
	...
	@:optional var bitmapArray:Array<String>;
...
class Character extends FlxSprite
{
	...
	**/
	...
	var oldScale:FlxPoint = FlxPoint.get(1, 1);
	...
	public function loadCharacterFile(json:Dynamic)
	{
		...
		if(!isAnimateAtlas){
			var fooFrames = Paths.shrunkSparrow(json.image, ClientPrefs.data.graphics_power);
			final bitmapArray:Null<Array<String>> = json.bitmapArray;
			if (json.bitmapArray != null && json.bitmapArray.length > 0)
			{
				for (bitmap in bitmapArray)
				{
					fooFrames.addAtlas(Paths.shrunkSparrow(bitmap, ClientPrefs.data.graphics_power));
				}
			}
			frames = fooFrames;
		}
	...
	function _updateScale(point:FlxPoint)
	{
