...
class PlayState extends MusicBeatState
{
	...
	public static var dadMania:Int = 4;
	public static var playerMania:Int = 4;
	public var shieldSystem:Bool = false;
	...
	public var gfBlock:Bool = false;
	var oldChar:String = 'bf';

	override public function create()
	{
		dadMania = playerMania = 4;
		...
		if (SONG.dadMania != null)
			dadMania = SONG.dadMania;
		if (SONG.playerMania != null)
			playerMania = SONG.playerMania;
		if (SONG.shieldSystem != null)
			shieldSystem = SONG.shieldSystem;
		switch (playerMania)
		{
			case 4:
			keysArray = [
		...
		switch (curStage)
		{
			...
			case 'sewers': new states.stages.Sewers();
			case 'garyHome': new states.stages.GaryHome();
			case 'cave': new states.stages.Cave();
			case 'sewers2': new states.stages.SewersSequel();
			case 'skyline': new states.stages.Skyline();
		}
	...
	private function generateSong(dataPath:String):Void
	{
		...
		try
		{
			if (songData.needsVoices)
				vocals.loadEmbedded(Paths.voices(songData.song + '/' + storyDifficulty + '-' + songData.song));
		}
		catch(e:Dynamic) {}
		...
		inst = new FlxSound();
		try {
			inst.loadEmbedded(Paths.inst(songData.song + '/' + storyDifficulty + '-' + songData.song));
		}
		catch(e:Dynamic) {}
		...
		for (section in noteData)
		{
			for (songNotes in section.sectionNotes)
			{
				...
				if (songNotes[1] > (section.mustHitSection ? playerMania : dadMania))
				{
					gottaHitNote = !section.mustHitSection;
				}
				final side:Int = section.mustHitSection ? playerMania : dadMania;
				var daNoteData:Int = Std.int(songNotes[1] - (songNotes[1] >= side ? side : 0));
				...
				var swagNote:Note = new Note(daStrumTime, daNoteData, oldNote, false, false, null, gottaHitNote);
				...
				if(floorSus > 0) {
					for (susNote in 0...floorSus + 1)
					{
						...
						var sustainNote:Note = new Note(daStrumTime + (Conductor.stepCrochet * susNote), daNoteData, oldNote, true, false, null, gottaHitNote);
				...
				{
					songData.events.push([songNotes[0], [[songNotes[2], songNotes[3], songNotes[4]]]]);
				}
			}
		}
	...
	private function generateStaticArrows(player:Int):Void
	{
		...
		for (i in 0...(player == 1 ? playerMania : dadMania))
		{
			// FlxG.log.add(i);
	...
	public function moveCamera(isDad:Bool)
	{
		if(isDad)
		{
			final point = generateCamPos('dad');
			camFollow.setPosition(point.x, point.y);
			tweenCamIn();
		}
		else
		{
			final point = generateCamPos('boyfriend');
			camFollow.setPosition(point.x, point.y);
	...
	public function generateCamPos(character:String = 'gf'):FlxPoint
	{
		if (oldChar == character){
			trace(character);
			return FlxPoint.weak(camFollow.x, camFollow.y);
		}
		
		var endPoint = FlxPoint.weak();
		switch (character){
			case 'dad':
			{
				dad.getMidpoint(endPoint);

				switch (curStage)
				{
					case 'sewers' | 'sewers2': 
						endPoint.set(dad.x + 325, dad.y + 150);
					case 'cave' | 'garyhome':
						endPoint.y -= 100;
						endPoint.x += 400;
				}
				endPoint.x += dad.cameraPosition[0];
				endPoint.y += dad.cameraPosition[1];
				
			} 
			case 'boyfriend':
			{
				endPoint.set(boyfriend.getMidpoint().x - 100, boyfriend.getMidpoint().y - 100);

				switch (curStage)
				{
					case 'cave':
						endPoint.y = dad.getMidpoint().y + dad.cameraPosition[1] - 75;
					case 'sewers':
						endPoint.x = boyfriend.getMidpoint().x - 450;
					case 'sewers2': 
						endPoint.y -= 20;
						
				}
				endPoint.x -= boyfriend.cameraPosition[0];
				endPoint.y += boyfriend.cameraPosition[1];
			}
			case 'gf':
			{
				gf.getMidpoint(endPoint);
				endPoint.x += gf.cameraPosition[0];
				endPoint.y += gf.cameraPosition[1];
				switch (curStage)
				{
					case 'sewers':
					endPoint.x -= 100;
			...
			if (isStoryMode)
			{
				...
				if (storyPlaylist.length <= 0)
				{
					...
					if(!ClientPrefs.getGameplaySetting('practice') && !ClientPrefs.getGameplaySetting('botplay')) {
						...
						CoolUtil.weekSpot[storyWeek] = 0; 
						FlxG.save.data.weekSpot = CoolUtil.weekSpot;
	...
	function noteMissCommon(direction:Int, note:Note = null)
	{
		...
		if(char != null && (note == null || !note.noMissAnimation) && char.hasMissAnimations)
		{
			...
			if(char != gf && lastCombo > 5 && gf != null && gf.animOffsets.exists('sad') && !gfBlock)
			{
				gf.playAnim('sad');
				gf.specialAnim = true;
			}
	...
	public function goodNoteHit(note:Note):Void
	{
		...
		if(!note.noAnimation) {
			...
			if (note.noteType.startsWith('open') || note.noteType == 'closed') 
				note.animSuffix = "-" + note.noteType;
	...
	public function spawnNoteSplash(x:Float, y:Float, data:Int, note:Note) {
		...
		final newData = CoolUtil.keyHandler(data, note.mustPress ? playerMania : dadMania);
		splash.setupNoteSplash(x, y, newData, note);
		grpNoteSplashes.add(splash);
	}
