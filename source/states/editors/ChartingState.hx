...
import states.PlayState.dadMania;
import states.PlayState.playerMania;
...
class ChartingState extends MusicBeatState
{
	...
	**/
	...
	override function create()
	{
		...
		// Paths.clearMemory();
		GRID_SIZE = Std.int(40 + 4 - 4 * (8/(playerMania + dadMania)));
		side = _song.notes[curSec].mustHitSection ? playerMania : dadMania;
		#if desktop
		...
		strumLine = new FlxSprite(0, 50).makeGraphic(Std.int(GRID_SIZE * (dadMania + playerMania + 1)), 4);
		add(strumLine);
		...
		for (i in 0...dadMania + playerMania){
			var note:StrumNote = new StrumNote(GRID_SIZE * (i+1), strumLine.y, i - (i >= side ? side : 0), 0);
	...
	function addSectionUI():Void
	{
		...
		var swapSection:FlxButton = new FlxButton(10, check_notesSec.y + 40, "Swap section", function()
		{
			...
			{
				var note:Array<Dynamic> = _song.notes[curSec].sectionNotes[i];
				note[1] = (note[1] + side) % (playerMania + dadMania);
		...
		var mirrorButton:FlxButton = new FlxButton(duetButton.x + 100, duetButton.y, "Mirror Notes", function()
		{
			...
			for (note in _song.notes[curSec].sectionNotes)
			{
				var boob = note[1] - (note[1] >= side ? side : 0);
				boob = side - 1 - boob;
				if (note[1] >= side) boob += side;
	...
	function loadSong():Void
	{
		...
		try
		{
			var file:Dynamic = Paths.voices(currentSongName + '/' + PlayState.storyDifficulty + '-' + currentSongName);
	...
	function generateSong() {
		FlxG.sound.playMusic(Paths.inst(currentSongName + '/' + PlayState.storyDifficulty + '-' + currentSongName), 0.6/*, false*/);
	...
	override function update(elapsed:Float)
	{
		...
		curRenderedNotes.forEachAlive(function(note:Note) {
			...
			if(curSelectedNote != null) {
				...
				if(noteDataToCheck > -1 && note.mustPress != _song.notes[curSec].mustHitSection) noteDataToCheck += side;
			...
			if(note.strumTime <= Conductor.songPosition) {
				...
				if(note.strumTime > lastConductorPos && FlxG.sound.music.playing && note.noteData > -1) {
					var data:Int = note.noteData - (note.noteData >= side ? side : 0);
					var noteDataToCheck:Int = note.noteData;
					if(noteDataToCheck > -1 && note.mustPress != _song.notes[curSec].mustHitSection) noteDataToCheck += side;
					...
					if(!playedSound[data]) {
						if(note.hitsoundChartEditor && ((playSoundBf.checked && note.mustPress) || (playSoundDad.checked && !note.mustPress)))
						{
							...
							FlxG.sound.play(Paths.sound(soundToPlay)).pan = note.noteData < side? -0.3 : 0.3; //would be coolio
	...
	*/
	...
	var columns:Int = dadMania + playerMania + 1;
	function reloadGridLayer() {
		...
		var gridBlackLine:FlxSprite = new FlxSprite(gridBG.x + gridBG.width - (GRID_SIZE * side)).makeGraphic(1, 1, FlxColor.BLACK);
	...
	function updateWaveform() {
		#if desktop
		if(waveformPrinted) {
			var width:Int = Std.int(GRID_SIZE * playerMania + dadMania);
		...
		// Draws
		var gSize:Int = Std.int(GRID_SIZE * playerMania+ dadMania);
	...
	function updateGrid():Void
	{
		...
		for (i in _song.notes[curSec].sectionNotes)
		{
			...
			if(i[1] >= side) note.mustPress = !note.mustPress;
		}
	...
	function setupNoteData(i:Array<Dynamic>, isNextSection:Bool):Note
	{
		...
		var note:Note = new Note(daStrumTime, daNoteInfo - (daNoteInfo >= side ? side : 0), null, null, true, null, false);
	...
	function selectNote(note:Note):Void
	{
		...
		{
			if(note.mustPress != _song.notes[curSec].mustHitSection) noteDataToCheck += side;
			for (i in _song.notes[curSec].sectionNotes)
			{
	...
	public function doANoteThing(cs, d, style){
		...
		if(strumLineNotes.members[d].overlaps(curRenderedNotes))
		{
			curRenderedNotes.forEachAlive(function(note:Note)
			{
				if (note.overlapsPoint(new FlxPoint(strumLineNotes.members[d].x + 1,strumLine.y+1)) && note.noteData == d - (d >= side? side : 0))
				{
						//trace('tryin to delete note...');
	...
	private function addNote(strum:Null<Float> = null, data:Null<Int> = null, type:Null<Int> = null):Void
	{
		...
		{
			_song.notes[curSec].sectionNotes.push([noteStrum, (noteData + side) % playerMania + dadMania, noteSus, curNoteTypes[daType]]);
		}
