...
class EditorPlayState extends MusicBeatSubstate
{
	...
	function generateSong(dataPath:String)
	{
		...
		vocals = new FlxSound();
		if (songData.needsVoices) vocals.loadEmbedded(Paths.voices(songData.song + '/' + PlayState.storyDifficulty + '-' + songData.song));
		vocals.volume = 0;

		#if FLX_PITCH vocals.pitch = playbackRate; #end
		FlxG.sound.list.add(vocals);

		inst = new FlxSound().loadEmbedded(Paths.inst(songData.song + '/' + PlayState.storyDifficulty + '-' + songData.song));
		...
		for (section in noteData)
		{
			for (songNotes in section.sectionNotes)
			{
				...
				if (songNotes[1] > (section.mustHitSection ? PlayState.playerMania : PlayState.dadMania))
				{
					gottaHitNote = !section.mustHitSection;
				}
				final side:Int = section.mustHitSection ? PlayState.playerMania : PlayState.dadMania;
				...
				var swagNote:Note = new Note(daStrumTime, daNoteData, oldNote, false, false, this, gottaHitNote);
				...
				if(floorSus > 0) {
					for (susNote in 0...floorSus + 1)
					{
						...
						var sustainNote:Note = new Note(daStrumTime + (Conductor.stepCrochet * susNote), daNoteData, oldNote, true, this, gottaHitNote);
	...
	private function generateStaticArrows(player:Int):Void
	{
		...
		for (i in 0...(player == 1 ? PlayState.playerMania : PlayState.dadMania))
		{
			// FlxG.log.add(i);
