...
import options.OptionsState.usedSystem;
...
class NotesSubState extends MusicBeatSubstate
{
		...
		else if(controls.RESET && hexTypeNum < 0)
		{
			if(FlxG.keys.pressed.SHIFT || FlxG.gamepads.anyJustPressed(LEFT_SHOULDER))
			{
				for (i in 0...3)
				{
					...
					var color:FlxColor = !onPixel ? ClientPrefs.defaultData.arrowRGB[usedSystem][curSelectedNote][i] :
													ClientPrefs.defaultData.arrowRGBPixel[usedSystem][curSelectedNote][i];
					switch(i)
					{
						case 0:
			...
			setShaderColor(!onPixel ? ClientPrefs.defaultData.arrowRGB[usedSystem][curSelectedNote][curSelectedMode] : ClientPrefs.defaultData.arrowRGBPixel[usedSystem][curSelectedNote][curSelectedMode]);
			FlxG.sound.play(Paths.sound('cancelMenu'), 0.6);
			updateColors();
		}
	}
	...
	{
		dataArray = !onPixel ? ClientPrefs.data.arrowRGB[usedSystem] : ClientPrefs.data.arrowRGBPixel[usedSystem];
		...
		{
			Note.initializeGlobalRGBShader(i, usedSystem);
			var newNote:StrumNote = new StrumNote(150 + (480 / dataArray.length * i), 200, i, 0);
			newNote.useRGBShader = true;
			newNote.setGraphicSize(408/usedSystem);
			newNote.updateHitbox();
			newNote.ID = i;
			newNote.y = 200 + 51 - (newNote.height / 2);
			myNotes.add(newNote);
		}

		bigNote = new Note(0, 0, false, true, null, false);
		bigNote.setPosition(250, 325);
		bigNote.setGraphicSize(250);
		bigNote.updateHitbox();
		bigNote.rgbShader.parent = Note.globalRgbShaders[curSelectedNote];
		bigNote.shader = Note.globalRgbShaders[curSelectedNote].shader;
		for (i in 0...usedSystem)
		{
			final newData = CoolUtil.keyHandler(i, usedSystem);
			if(!onPixel) bigNote.animation.addByPrefix('note$i', Note.colArray[newData] + '0', 24, true);
			else 
				newData == 4 ? bigNote.animation.addByPrefix('note$i', 'shield0', 24, true): 
					bigNote.animation.add('note$i', [newData + 4], 24, true);
		}
	...
	function updateNotes(?instant:Bool = false)
	{
		...
		for (note in myNotes)
		{
			...
			if(note.animation.curAnim == null || note.animation.curAnim.name != newAnim){ 
				...
				note.setGraphicSize(408/usedSystem);
				note.updateHitbox();
			}
			if(instant && note.animation.curAnim != null) note.animation.curAnim.finish();
		}
		final animToPlay = curSelectedNote;
		bigNote.animation.play('note$animToPlay', true);
		bigNote.setGraphicSize(250);
		bigNote.updateHitbox();
		updateColors();
	}
